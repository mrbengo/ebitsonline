...
function AddRecord($action) {
	...
	<tr>
	  <td align="right" class="formscss">Due Date :*</td>
	  <td><table width="55%"  border="0" cellspacing="0" cellpadding="0">
		  <tr>
			<td><select name="thedate1">
				<?PHP 				
				for ($q=1; $q<=31; $q++)
				{
					echo '<option value="'.$q.'"'; echo ' />'.$q.'</option>';echo "\n";
				}
				?>
			  </select>
			</td>
			<td><select name="themonth1">
				<?PHP 				
				for ($r=1; $r<=12; $r++)
				{
					echo '<option value="'.$r.'"'; echo ' />'.$r.'</option>';echo "\n";
				}
				?>
			  </select>
			</td>
			<td><select name="theyear1">
				<?PHP 				
				for ($s=2005; $s<date('Y')+1; $s++)
				{
					echo '<option value="'.$s.'"'; echo ' />'.$s.'</option>';echo "\n";
				}
				?>
			  </select>
			</td>
...
function EditRecord($action, $strID) {
	...
	<tr>
	  ...
	  <td><table width="55%"  border="0" cellspacing="0" cellpadding="0">
		  <tr>
			<td><select name="thedate1" disabled="disabled" >
				<?PHP 				
				for ($q=1; $q<=31; $q++)
				{
					echo '<option value="'.$q.'"';if($q == "$strThedate1"){echo "Selected";}echo ' />'.$q.'</option>';echo "\n";
				}
				?>
			  </select>
			</td>
			<td><select name="themonth1" disabled="disabled" >
				<?PHP 				
				for ($r=1; $r<=12; $r++)
				{
					echo '<option value="'.$r.'"';if($r == "$strThemonth1"){echo "Selected";}echo ' />'.$r.'</option>';echo "\n";
				}
				?>
			  </select>
			</td>
			<td><select name="theyear1" disabled="disabled" >
				<?PHP 				
				for ($s=2005; $s<date('Y')+1; $s++)
				{
					echo '<option value="'.$s.'"';if($s == "$strTheyear1"){echo "Selected";}echo ' />'.$s.'</option>';echo "\n";
				}
				?>
			  </select>
			</td>
...
function ProcessAddForm($formValues) {
	...
	$strCompany = preg_replace('/\'/', "''", trim($formValues["company"]));
		...
		$strInvoiceDate = $strYear."-".$strMonth."-".$strTDate;

		//Create the Due Date
		$strTDate1 = preg_replace('/\'/', "''", trim($formValues["thedate1"]));
		$strMonth1 = preg_replace('/\'/', "''", trim($formValues["themonth1"]));
		$strYear1 = preg_replace('/\'/', "''", trim($formValues["theyear1"]));
		$strDuedate = $strYear1."-".$strMonth1."-".$strTDate1;
