...
if ($uzuriweb_log_val <> "")
{
	...
	$strPath = "";
	$strTitle = 'Expenses (Money Out) Report';
...
<html>
<head>
<title>Expenses (Money Out) Report</title>
...
<table width="100%" border="0" cellspacing="2" cellpadding="2">
  ...
  <tr>
    <td colspan="3" align="center" class="titletx">Expenses Report</td>
  </tr>
</table>
...
<?
	...
	if (isset($_GET['PB'])) //if input from form
	{
		...
		$strExpensetype = preg_replace('/\'/', "''", strip_tags(htmlspecialchars($_POST["expensetype"])));
		
		$query="SELECT * FROM wbm_expenses WHERE companyid=$strWbmCompanyid";
		if(($strDatefrom)&&($strDateto))
			{
				$query = $query." AND paiddate BETWEEN '$strDatefrom' AND '$strDateto'"; 
			}	
		if($strExpensetype <> 0)
			{
				$query = $query." AND category=$strExpensetype"; 
			}	
	...
	echo "<td width=\"35%\" class=\"frmHeader\"><b>Paid To</b></td>\n";
	echo "<td width=\"25%\" class=\"frmHeader\"><b>Category</b></td>\n";
	echo "<td width=\"15%\" class=\"frmHeader\"><b>Amount</b></td>\n";
	echo "</tr>\n";
