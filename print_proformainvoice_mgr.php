...
if ($uzuriweb_log_val <> "")
{
	...
	$sqlinve = "SELECT * FROM wbm_pro_invoices WHERE ID=$strID LIMIT 1";
	$resultinve = mysql_query($sqlinve) or die(mysql_error());
	$rowinve = mysql_fetch_array($resultinve);
	$strThetitle = $rowinve['invoiceNum']; 	
...
<table width="100%" border="0" cellspacing="2" cellpadding="2">
  ...
  <tr>
	...
	$sqlname = "SELECT * FROM wbm_pro_invoices WHERE ID=$strID LIMIT 1";
	...
	?>
	<h1><? echo $strTitle;?></h1>
	<span class="inputheader">NUMBER #:</span> &nbsp;<? echo $strTheInvoice;?><br>
	<span class="inputheader">DATE:</span> &nbsp;<? echo $strInvoiceDate;?><br>
	<span class="inputheader">TERMS:</span> &nbsp;Payment within 21 days <br>
...
<br>
	...
	echo "</tr>\n";
	$query  = "SELECT * FROM wbm_invoicedetails WHERE invoiceNum = '$strTheInvoice' AND companyid=$strWbmCompanyid ORDER BY ID ASC ";
	...
	echo "<td class=\"inputheader\" colspan=\"2\">SUB TOTAL:</td>\n";
		...
		$strNewSubTotal = number_format($strNewSubTotal, 2,".",",");
		echo $strNewSubTotal; 
	...
	echo "<td>&nbsp;</td>\n";
	echo "<td>&nbsp;</td>\n";
	echo "<td class=\"inputheader\" colspan=\"2\">\n";
	echo $strDiscount."% Discount:</td>\n";
		echo "<td class=\"frmTabletxt2\" align=\"right\">\n";
		$strDiscountamnt = number_format($strDiscountamnt, 2,".",",");
		echo $strDiscountamnt;
		//echo formatMoney($strDiscountamnt); 
		echo "</td>\n";
	echo "</tr>\n";
	}
