...
if ($uzuriweb_log_val <> "")
{
	$uzuriwebpgname="printreceipt_mgr";
	$strPath = "";
	$strTitle = 'RECEIPT';
	
	$strID = rtrim($_GET['receiptid']);
	...
	$sqlinve = "SELECT * FROM wbm_receipts WHERE ID=$strID LIMIT 1";
	$resultinve = mysql_query($sqlinve) or die(mysql_error());
	$rowinve = mysql_fetch_array($resultinve);
	$strThetitle = $rowinve['receiptNum']; 	
...
<table width="100%" border="0" cellspacing="2" cellpadding="2">
  ...
  <tr>
	...
	$sqlname = "SELECT * FROM wbm_receipts WHERE ID=$strID LIMIT 1";
	$resultname = mysql_query($sqlname) or die(mysql_error());
	$rowname = mysql_fetch_array($resultname);
	$strTheInvoice = $rowname['ID']; 	
	$strETRNum = $rowname['receiptNum'];
	$strInvoiceNum = $rowname['invoiceNum'];
	$strProInvoice = $rowname['proinvoiceNum'];
	...
	?>
	<h1><? echo $strTitle;?></h1>
	<h2>Receipt No#: <? echo $strETRNum;?></h2>
	<span class="inputheader">RECEIPT DATE:</span> &nbsp;<? echo $strInvoiceDate;?><br>
	<span class="inputheader">INVOICE No#:</span> &nbsp;<? echo $strInvoiceNum;?><br>
...
<br>
	...
	echo "<td width=\"15%\" class=\"frmHeader\" align=\"right\"><b>Total(ksh)</b></td>\n";
	echo "</tr>\n";
	
	echo "<tr>\n";
	echo "<td class=\"frmTabletxt3\" valign=\"top\" align=\"right\"> 1 </td>\n";
	echo "<td class=\"frmTabletxt3\" valign=\"top\">\n";

	$query  = "SELECT itemDesc FROM wbm_invoicedetails WHERE invoiceNum = '$strProInvoice' AND companyid=$strWbmCompanyid ORDER BY ID ASC ";
	...
	echo "</td>\n";
	$strInvoiceAmount11 = number_format($strInvoiceAmount, 2,".",",");
	echo "<td class=\"frmTabletxt3\" valign=\"top\">1</td>\n";
	echo "<td class=\"frmTabletxt3\" valign=\"top\" align=\"right\">";
	echo $strInvoiceAmount11; 
	echo "</td>\n";
	echo "<td class=\"frmTabletxt3\" valign=\"top\" align=\"right\">";
	echo $strInvoiceAmount11; 
	echo "</td>\n";
	...
	echo "</tr>\n";
	echo "<tr>\n";
	echo "<td colspan=7>&nbsp;<h2>E&OE</h2></td>\n";
	echo "</tr>\n";
	echo "</table>\n";
	echo "</form>\n";
	echo '<br>';
