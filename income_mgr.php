...
if ($uzuriweb_log_val <> "")
{
	...
	$_SESSION['uzuriweb_income_search']="Nosearch";
	header('Location: income_mgr.php');
	}
	
	$uzuriwebpgname="income_mgr";
	$strPath = "";
	$strTitle = 'Manage Income (Money In)';
	$strID = rtrim($_GET['ID']);
	$strTheClientID = rtrim($_GET['clientid']);
	$strInvID = rtrim($_GET['invid']);
	
	//Define Invoice Steps
	$strThestep = rtrim($_GET['step']);
	if ($strThestep==""){
	$_SESSION['uzuriweb_wbm_steps']="stepone";
	}
	else{
	$_SESSION['uzuriweb_wbm_steps']=$strThestep;
	}
	//end of steps
	...
	elseif ($strMode=="other"){
	$_SESSION['uzuriweb_wbm_modes']="other";
	}
	elseif ($strMode=="edit"){
	...
	if ((isset($_GET['search']) && ($_GET['search'])=='y')){
	    $_SESSION['uzuriweb_income_search']="Search";
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
	  ...
	  <tr>
	    <td colspan="2" valign="top">
	      <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td width="48%" valign="top" class="breadcrumbs"><strong>YOU ARE HERE:</strong> <a href="dashboard.php">Dashboard</a>&nbsp;->&nbsp;Manage Income (Money In)</td>
              <td width="52%" align="right" class="breadcrumbs"><strong>COMPANY:</strong>  <?php echo WEB_SITE_NAME;?> &nbsp;&nbsp;<strong>LOGGED IN AS:</strong>  <?php echo $_SESSION['uzuriweb_wbm_username'];?> &nbsp;&nbsp;</td>
            </tr>
          </table>
		  </td>
        </tr>
		...
		<tr>
          ...
          <tr>
            ...
            <td valign="top">
				...
				$strSteps=$_SESSION['uzuriweb_wbm_steps'];
				if ($strAction=="view"){
				LoadRecords();
				}
				elseif ($strAction=="invoice"){
				AddInvoiceRecord("new", $strSteps, $strTheClientID, $strInvID);
				}
				elseif ($strAction=="other"){
				AddOtherRecord("new");
				}
