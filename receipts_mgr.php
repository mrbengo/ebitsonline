...
$_SESSION['uzuriweb_receipts_search'];
$_SESSION['uzuriweb_wbm_modes'];
$_SESSION['receiptcode'];
$_SESSION['calc_tax'];

//Include common files
include('includes/wbm_common.php');
include('functions/receipts_fns.php');
...
if ($uzuriweb_log_val <> "")
{
	...
	$_SESSION['uzuriweb_receipts_search']="Nosearch";
	header('Location: receipts_mgr.php');
	}
	
	$uzuriwebpgname="receipts_mgr";
	$strPath = "";
	$strTitle = 'Manage Receipts';
	
	$strID = rtrim($_GET['ID']);
	$strReceiptID = rtrim($_GET['inv']);
	$strMode = rtrim($_GET['mode']);
	if ($strMode==""){
	$_SESSION['uzuriweb_wbm_modes']="view";
	}
	elseif ($strMode=="receipt"){
	$_SESSION['uzuriweb_wbm_modes']="receipt";
	}
	//End of Setting action modes
	
	if ((isset($_GET['search']) && ($_GET['search'])=='y')){
	    $_SESSION['uzuriweb_receipts_search']="Search";
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
	  ...
	  <tr>
	    <td colspan="2" valign="top">
	      <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td width="63%" valign="top" class="breadcrumbs"><strong>YOU ARE HERE:</strong> <a href="dashboard.php">Dashboard</a>&nbsp;->&nbsp;Manage Receipts</td>
